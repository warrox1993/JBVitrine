/**
 * Smidjan brand mark — a monoline emerald "S" (the "smid"/forge idea) with a
 * gold forge-ember at its base. Bare (no plate) so the emerald reads on both
 * light and dark surfaces; inline SVG → crisp at every size, no raster asset.
 * The favicon counterpart lives in `src/app/icon.svg`.
 */
export function BrandMark({
  size = 40,
  className,
}: {
  size?: number;
  className?: string;
}) {
  return (
    <svg
      className={className}
      width={size}
      height={size}
      viewBox="0 0 40 40"
      fill="none"
      aria-hidden="true"
    >
      <path
        d="M27.2 13.2C27.2 10.3 23.4 8.6 19.8 8.6C15.5 8.6 12.4 10.9 12.4 14.1C12.4 17.2 15.5 18.6 19.9 19.4C24.3 20.2 27.6 21.5 27.6 25.1C27.6 28.8 24.2 31.2 19.8 31.2C16.1 31.2 13.1 29.6 12.4 26.8"
        stroke="#13A277"
        strokeWidth="6.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M16.5 20.5C13.5 24.5 11.8 27.5 12.4 30.8C12.9 33.8 14.6 35.8 16.5 35.8C18.4 35.8 20.1 33.8 20.6 30.8C21.2 27.5 19.5 24.5 16.5 20.5Z"
        fill="#F0D691"
      />
      <path
        d="M16.5 25C14.9 27.4 14.4 29.5 14.8 31.4C15.2 33.1 15.8 34.3 16.5 34.3C17.2 34.3 17.8 33.1 18.2 31.4C18.6 29.5 18.1 27.4 16.5 25Z"
        fill="#E8952B"
      />
    </svg>
  );
}

export default BrandMark;
