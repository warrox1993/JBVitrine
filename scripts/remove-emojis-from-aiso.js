...
const originalContent = aisoArticle.content;
...
aisoArticle.content = cleanedContent;

// Count removed emojis
const originalEmojis = originalContent.match(emojiPattern);
const removedCount = originalEmojis ? originalEmojis.length : 0;

// Write back to file
fs.writeFileSync(filePath, JSON.stringify(data, null, 2), "utf8");

console.log("✅ Emojis removed from AISO article!");
console.log(`📊 Total emojis removed: ${removedCount}`);
console.log(
  `📝 Content length: ${originalContent.length} → ${cleanedContent.length} characters`,
);
